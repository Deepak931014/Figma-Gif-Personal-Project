...
const Gallery = ({ searchTerm }) => {
  ...
  return (
    ...
    <div className="w-full max-w-7xl mx-auto h-[600px] overflow-y-scroll scrollbar-hidden">
        ...
        {filteredImages.length > 0 ? (
          filteredImages.map((image) => (
            <Link
              key={image.id}
              to={`/details/${image.id}`}
              className="w-full h-80 bg-gray-200 rounded-lg shadow-lg cursor-pointer hover:opacity-80 transition relative"
            >
              <img
                src={image.src}
                alt={image.name}
          ...
          <p className="text-gray-500 text-lg col-span-4">No matching results found</p>
        )}
      </div>
    </div>
  </div>
